    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex justify-center items-center gap-4 sm:gap-8 my-4 sm:my-8 relative min-h-[150px] sm:min-h-[200px]">
            {(step === 1 || step === 2) && (
              <div className="relative flex flex-col items-center">
                {/* Scale bar */}
                <motion.div
                  className="w-full sm:w-48 h-2 bg-border rounded-full mb-4"
                  animate={balanced ? { scale: [1, 1.05, 1] } : {}}
                />
                
                {/* Scale platform */}
                <div className="flex items-start justify-center gap-8 sm:gap-12 md:gap-16 relative">
                  {/* Left side (numerator) */}
                  <div className="flex flex-col items-center">
                    ...
                    />
                    <motion.div
                      className="w-16 h-16 sm:w-24 sm:h-24 rounded-full bg-primary/20 border-2 sm:border-4 border-primary flex items-center justify-center relative"
                      animate={balanced ? { scale: 1.1 } : {}}
                    >
                      <MathText className="text-base sm:text-xl font-bold">5</MathText>
                      {balanced && (
                        <motion.div
                          className="absolute -top-6 text-base sm:text-xl font-bold text-gem"
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    />
                    <motion.div
                      className="w-16 h-16 sm:w-24 sm:h-24 rounded-full bg-secondary/20 border-2 sm:border-4 border-secondary flex items-center justify-center relative"
                      animate={balanced ? { scale: 1.1 } : {}}
                    >
                      <MathText className="text-base sm:text-xl font-bold">2</MathText>
                      {balanced && (
                        <motion.div
                          className="absolute -top-6 text-base sm:text-xl font-bold text-gem"
                          initial={{ opacity: 0, y: 10 }}
                ...
                {!balanced && (
                  <motion.div
                    className="absolute -top-8 text-2xl sm:text-3xl md:text-4xl font-bold text-primary"
                    animate={{ y: [0, -5, 0] }}
            ...
            {step === 3 && (
              <div className="flex flex-col items-center gap-2">
                <motion.div
                  className="w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-primary/20 border-2 sm:border-4 border-primary flex items-center justify-center glow"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                >
                  <MathText className="text-lg sm:text-2xl font-bold">5³</MathText>
                </motion.div>
                <div className="h-1 w-24 bg-muted-foreground rounded" />
                <motion.div
                  className="w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-secondary/20 border-2 sm:border-4 border-secondary flex items-center justify-center glow"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
      ...
      <div className="flex justify-center gap-2 sm:gap-4">
        {currentStep.action === "interact" && !balanced ? (
          <Button onClick={handleBalance} size="lg" className="glow text-sm sm:text-base h-10 sm:h-11 px-4 sm:px-6">
            Distribute Power
          </Button>
        ) : (
          <Button onClick={handleNext} size="lg" disabled={balanced && step === 2} className="text-sm sm:text-base h-10 sm:h-11 px-4 sm:px-6">
