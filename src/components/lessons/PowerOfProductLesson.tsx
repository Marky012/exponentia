    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      >
        ...
        {currentStep.visual && (
          <div className="flex justify-center items-center gap-4 sm:gap-8 my-4 sm:my-8 relative min-h-[150px] sm:min-h-[200px] overflow-hidden">
            {step === 1 && (
              <motion.div
                className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-primary/20 border-2 sm:border-4 border-primary flex items-center justify-center glow relative"
              >
                <MathText className="text-xl sm:text-3xl font-bold">(2×3)⁴</MathText>
            ...
            {step === 2 && (
              <div className="relative w-full flex items-center justify-center">
                {!split ? (
                  ...
                  <>
                    <motion.div
                      className="w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-primary/20 border-2 sm:border-4 border-primary flex items-center justify-center absolute"
                      initial={{ x: 0 }}
                      animate={{ x: -80 }}
                    >
                    ...
                    <motion.div
                      ...
                      animate={{ x: 80 }}
                    >
                      <MathText className="text-lg sm:text-2xl font-bold">3</MathText>
                    </motion.div>
                    <motion.div
                      className="absolute -top-4 sm:-top-8 left-1/2 -translate-x-1/2 text-xl sm:text-4xl font-bold text-gem"
      ...
      <div className="flex justify-center gap-2 sm:gap-4">
        {currentStep.action === "interact" && !split ? (
          <Button onClick={handleSplit} size="lg" className="glow text-sm sm:text-base h-10 sm:h-11 px-4 sm:px-6">
            Split the Cores
          </Button>
        ) : (
          <Button onClick={handleNext} size="lg" disabled={split && step === 2} className="text-sm sm:text-base h-10 sm:h-11 px-4 sm:px-6">
